...
import { Target, AlertTriangle, Zap, TrendingUp, TrendingDown, Activity } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="relative flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                ...
                {topHunt.induced && (
                  <Badge color="primary" label="⚡ اختراق مُزيّف" />
                )}
      ...
      <div className="flex flex-wrap items-center gap-3 text-[10px] text-muted-foreground px-1">
        ...
        <LegendItem color="text-primary" label="⚡ اختراق مزيّف (inducement)" />
...
function ZoneRow({ zone: z, isAbove, mid }: { zone: LiquidityZone; isAbove: boolean; mid: number }) {
  ...
  return (
    <div className={cn(
      ...
      {/* Row 1: touches badge + price + probability */}
      ...
      {/* Row 2: distance + volume bar + tags */}
...
function Badge({ color, label }: { color: "gold" | "bull" | "bear" | "primary"; label: string }) {
  const cls = {
  ...
  return (
    <span className={cn("text-[9px] px-1.5 py-0.5 rounded-full border font-medium", cls)}>
      {label}
    </span>
  );
}
