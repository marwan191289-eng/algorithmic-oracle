...
export function useLiveDepth(symbol: string) {
  ...
  useEffect(() => {
    ...
    // Seed with REST snapshot so first paint is rich (500 levels).
    ...
    const connect = () => {
      ...
      ws.onmessage = (e) => {
        ...
        try {
          ...
          setBook((prev) => {
            // Merge top 20 live levels over the 500-level REST snapshot for
            // walls detection deeper in the book.
